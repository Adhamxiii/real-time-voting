    ...
    socket.on("room-update", (message: string) => {
      ...
      data.map((newWord) => {
        ...
        if (isWordAlreadyIncluded) {
          setWords((prev) => {
            const before = prev.filter((w) => w.text === newWord.text);
            const rest = prev.filter((w) => w.text !== newWord.text);

            return [
              ...rest,
              { text: before[0].text, value: before[0].value + newWord.value },
            ];
          });
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center min-h-screen bg-grid-zinc-50 pb-20">
      <MaxWidthWrapper className="flex flex-col items-center gap-6 pt-10">
        <h1 className="text-4xl font-bold sm:text-5xl text-center tracking-tight text-balance">
        ...
        <div className="aspect-square max-w-lg flex items-center justify-center">
          <Wordcloud
            words={words}
            width={500}
            height={500}
